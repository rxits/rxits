...
//   • live tech NEWS (Hacker News, free API)
//   • FACTS (free uselessfacts API + a curated tech pool)
//   • rxit's HOT TAKES (humour)
// Each slide shows ~6s. Fetches are best-effort; curated content is the fallback.
...
const TECH_FACTS = [
  'the first computer bug was a real moth, taped into a 1947 logbook.',
  ...
  "NASA's Apollo Guidance Computer had less power than a modern USB charger.",
  'the @ symbol was on typewriters back in the 1880s.',
  'about 90% of the world’s data was created in the last few years.',
  'the first webcam watched a coffee pot at Cambridge so no one walked over for nothing.',
];
...
async function collect() {
  ...
  const hn = await safe(async () => {
    const r = await fetch('https://hn.algolia.com/api/v1/search?tags=front_page&hitsPerPage=20', { headers: { 'User-Agent': 'rxits-profile' } });
    return (await r.json()).hits;
  }, []);
  hn.filter((h) => h.title).sort((a, b) => (b.points || 0) - (a.points || 0)).slice(0, 4)
    .forEach((h) => items.push({ type: 'news', text: cap(h.title, 90), meta: h.url ? domain(h.url) : 'news.ycombinator.com' }));

  // FACTS — free uselessfacts API (best-effort, a couple)
  for (let i = 0; i < 2; i++) {
    const f = await safe(async () => {
      const r = await fetch('https://uselessfacts.jsph.pl/api/v2/facts/random?language=en', { headers: { 'User-Agent': 'rxits-profile' } });
      return (await r.json()).text;
    }, null);
    if (f) items.push({ type: 'fact', text: cap(String(f).replace(/\s+/g, ' '), 90), meta: 'did you know' });
  }

  // FACTS — curated tech pool
  TECH_FACTS.slice().sort(() => Math.random() - 0.5).slice(0, 3).forEach((t) => items.push({ type: 'fact', text: cap(t, 90), meta: 'tech fact' }));

  // HOT TAKES — humour
  HOT_TAKES.slice().sort(() => Math.random() - 0.5).slice(0, 5).forEach((t) => items.push({ type: 'take', text: t, meta: '— rxit' }));

  return items.sort(() => Math.random() - 0.5).slice(0, 14);
}
...
function build(items) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 840 172" width="840" height="172" fill="none" role="img" aria-label="transmission — news, facts, hot takes">
  ...
  <g clip-path="url(#tw)">
    ...
    <text x="810" y="39" text-anchor="end" font-size="11" fill="#7f7fb0">news · facts · takes · auto-rotating</text>
